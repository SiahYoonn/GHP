...
the_connection = mavutil.mavlink_connection('/dev/serial0', baud=115200,
                                            dialect='ardupilotmega', autoreconnect=True) #oh???
      ...
      for box in result.boxes:
        ...
        if abs(error_x) > error_threshold or abs(error_y) > error_threshold:
          if error_x > error_threshold:
            send_ned_velocity(SOUTH, 0, 0, 1)
          elif error_x < -error_threshold:
            send_ned_velocity(NORTH, 0, 0, 1)
          if error_y > error_threshold:
            send_ned_velocity(0, EAST, 0, 1)
          elif error_y < -error_threshold:
            send_ned_velocity(0, WEST, 0, 1)
